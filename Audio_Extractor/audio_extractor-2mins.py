...
# Function to extract audio in 120-second chunks with fade-in and fade-out effects
def split_video_to_mp3_chunks(input_video_path, output_folder, chunk_duration=120, fade_duration=3):
    ...
    for chunk_idx in range(num_chunks):
        ...
        # FFmpeg command to extract 120-second audio chunk with fade effects
        command = [
            'ffmpeg',
            ...
            '-af', f"afade=in:st=0:d={fade_duration},afade=out:st={chunk_duration-fade_duration}:d={fade_duration}",  # Fade-in and fade-out filter
        ...
        try:
            ...
            print(f"Extracted and faded chunk: {output_audio_path}")
    ...
    for root, dirs, files in os.walk(input_folder):
        for file in files:
            if file.endswith(('.mp4', '.mov', '.avi', '.mkv', '.flv', '.webm')):  # Add other video extensions if necessary
                ...
                # Split the video into 120-second audio chunks with fade-in and fade-out effects
...
if __name__ == "__main__":
    ...
    # Start processing videos (120-second chunks with 3-second fade by default)
