...
def merge_audio(input_audios, output_file, target_duration):
    ...
    # 1. Convert audio files to 44100 Hz sample rate 
    converted_audios = []
    for i, audio_file in enumerate(input_audios):
        converted_audio_path = os.path.join(converted_folder, os.path.basename(audio_file) + "_converted.mp3")
        convert_audio_sample_rate(audio_file, converted_audio_path)
        converted_audios.append(converted_audio_path)

    # 2. Validate audio streams (after conversion)
    for i, audio_file in enumerate(converted_audios):
    ...
    merged_audios = []
    while total_duration < target_duration:
        random_audio = random.choice(converted_audios)
        merged_audios.append(random_audio)
        total_duration += get_file_duration(random_audio)

    # 4. Construct the FFmpeg command to concatenate audios
    ...
    # 5. Create a unique temporary output file
    temp_output_file = os.path.join(os.path.dirname(output_file), f"temp_merged_audio_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.mp3")  # Use a timestamp for uniqueness

    # 6. Concatenate audios into the temporary file
    ffmpeg_command = [
        'ffmpeg', 
        *inputs,
        '-filter_complex', filter_complex,
    ...
    # 7. Trim excess audio (if necessary)
    if total_duration > target_duration:
        trim_command = ['ffmpeg', '-i', temp_output_file, '-t', str(target_duration), '-c:a', 'copy', temp_output_file]  # Notice: temp_output_file is both input and output here, BUT we are trimming the file. FFmpeg does not overwrite in place when trimming.
    ...
    # 8. Move the trimmed audio to the final output file
    shutil.move(temp_output_file, output_file)  # Move the temporary file to the final output file

    # Remove the temporary file if the move was successful
...
def main():
    ...
    # Re-encode the merged and original audios
    ...
    merged_video_path = merge_videos(video_files, output_folder, original_audio_duration, target_resolution)

    # Merge the re-encoded audios to create the final audio
    final_audio_path = os.path.join(output_folder, "final_audio.mp3")
    merge_audio(audio_files, final_audio_path, original_audio_duration) # Fix the output_file path

    # Ensure the file exists before merging the final video
    if os.path.exists(final_audio_path):
        # Add the final audio to the merged video
        final_video_path = os.path.join(output_folder, "new_video.mp4")
        ffmpeg_command = [
            'ffmpeg',
            '-i', merged_video_path,   # Merged video from `merge_videos`
            '-i', final_audio_path,  # Your final audio
