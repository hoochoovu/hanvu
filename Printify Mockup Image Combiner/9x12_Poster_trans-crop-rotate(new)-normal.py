...
OFFSET_X = 910 #Higher number goes right, lower goes left
OFFSET_Y = 810 #Higher number goes down, lower number goes up
...
# Function to process and overlay images with rotation
def overlay_images(bg_folder, fg_folder, output_folder, output_format="jpg", fg_opacity=100, rotation_angle=0):
    ...
    for fg_file in fg_images:
        ...
        # Rotate the foreground image around its center
        print(f"Rotating {fg_file} by {rotation_angle} degrees...")
        fg_image = fg_image.rotate(rotation_angle, expand=False, center=(fg_image.width // 2, fg_image.height // 2))
        
        # Resize foreground image after rotation
...
if __name__ == "__main__":
    bg_folder = r"E:\Python_Practice\Printify Mockup Image Combiner\Background Templates\halloweenlight"
    fg_folder = "fg"
    ...
    # Set rotation angle for foreground image
    rotation_angle = 0  # Example: Rotate by 45 degrees

    overlay_images(bg_folder, fg_folder, output_folder, output_format, fg_opacity, rotation_angle)
