...
OFFSET_X = 1307  # Higher number moves left, Lower number moves right
OFFSET_Y = 353  # Higher number moves down, Lower number moves up

# Cropping size (pixels) for foreground image
CROP_LEFT = 0
CROP_RIGHT = 0
CROP_TOP = 20   # New variable for cropping from the top
CROP_BOTTOM = 25  # New variable for cropping from the bottom
...
# Function to crop an image on all sides (left, right, top, bottom)
...
def overlay_images(bg_folder, fg_folder, output_folder, output_format="jpg", fg_opacity=100):
    ...
    for fg_file in fg_images:
        ...
        # Crop foreground image (left, right, top, and bottom)
        resized_fg_width, resized_fg_height = fg_image.size
        cropped_fg_image = crop_image(fg_image, CROP_LEFT, CROP_RIGHT, CROP_TOP, CROP_BOTTOM)
...
if __name__ == "__main__":
    bg_folder = r"E:\Python_Practice\Printify Mockup Image Combiner\Background Templates\Canva\4-Canva"
    fg_folder = r"E:\Python_Practice\Printify Mockup Image Combiner\Canva Mockup Generator\canva_fg\5x7"
