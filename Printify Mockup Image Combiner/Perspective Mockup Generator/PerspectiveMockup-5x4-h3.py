...
def overlay_images(bg_folder, fg_folder, output_folder, output_format="jpg", fg_opacity=100):
    ...
    dst_top_left = (1052, 1110)
    dst_top_right = (3238, 1307)
    dst_bottom_right = (3365, 2990)
    dst_bottom_left = (1146, 3215)
    
    dst_points = [dst_top_left, dst_top_right, dst_bottom_right, dst_bottom_left]

    # Walk through fg_folder and all its subdirectories
    for root, _, files in os.walk(fg_folder):
        fg_images = [f for f in files if f.lower().endswith(('jpg', 'jpeg', 'png'))]
        ...
        for fg_file in fg_images:
            ...
            for bg_file in bg_images:
                ...
                # Ensure subfolder structure is mirrored in the output folder
                relative_path = os.path.relpath(root, fg_folder)
                fg_subfolder = os.path.join(output_folder, relative_path)
                if not os.path.exists(fg_subfolder):
                    os.makedirs(fg_subfolder)

                # Save the image in the corresponding output folder
...
if __name__ == "__main__":
    bg_folder = r"E:\Python_Practice\Printify Mockup Image Combiner\Perspective Mockup Generator\bg\h3"
