...
def process_image_top_left_bias(input_image, output_folder, ratio, output_format, use_transparency=False, dpi=None):
    ...
    with Image.open(input_image) as img:
        ...
        # Cropping box biased to the top-left (left=0, top=0)
        left = 350
        top = 0
...
def process_images_top_left_bias(input_folder, output_folder, output_format, use_transparency=False, dpi=None):
    ...
    for root, _, files in os.walk(input_folder):
        for image_file in files:
            ...
            for ratio_name, ratio in ratios.items():
                process_image_top_left_bias(input_image_path, file_output_folder, ratio, output_format, use_transparency, dpi)

if __name__ == '__main__':
    # Example usage
    input_folder = r'E:\ETSY ETSY ETSY\Design for Trends\Headless Horseman\Test Images\Final\ETSY FINAL\Fix'
    output_folder = 'output_top_left_bias'
    output_format = 'jpg'  # 'jpg' or 'png'
    use_transparency = True  # For png images only
    dpi = 300  # Set DPI, or None to preserve original DPI

    process_images_top_left_bias(input_folder, output_folder, output_format, use_transparency, dpi)
