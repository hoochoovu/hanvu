...
def process_video_sequence(sequence_folders, bg_folder, output_folder, quality="Default", zoom=1.0, x_offset=0, y_offset=0):
    """Processes a sequence of video folders, merges them with background videos, and outputs the final video."""
    used_videos = set()
    used_audios = set()
    ...
    for folder in sequence_folders:
        ...
        bg_videos = ensure_unique_bg_videos(bg_folder, used_videos, get_video_duration(fg_video_path))

        output_video_path = os.path.join(output_folder, f"output_{os.path.basename(folder)}.mp4")
        merge_videos(bg_videos, fg_video_path, output_video_path, zoom, x_offset, y_offset, quality)

        final_output_path = merge_audio_to_video(output_video_path, bg_folder, bg_music_volume=-5, final_audio_volume=-3, used_audios=used_audios)

        logging.info(f"Final output for folder {folder}: {final_output_path}")

# Example usage with the specified folder sequence
sequence_folders = ["title_folder", "fg_folder", "author_folder", "fg_folder", "stoistica_folder"]

def create_final_video_sequence(bg_folder, fg_folder, title_folder, author_folder, stoistica_folder, output_folder, resolution, duration, iterations, quality, bg_music, bg_music_volume, final_audio_volume, altbg_category=None):
    ...
    altbg_folder = None
    if altbg_category:
        altbg_folder = os.path.join(bg_folder, altbg_category)
    ...
    for i in range(iterations):
        try:
            ...
            # Build the video sequence in the correct order: FG, Author, FG, Stoistica
            folders_sequence = [fg_folder, author_folder, fg_folder, stoistica_folder]
            ...
            # Merge audio and truncate to the shortest duration
...
if __name__ == "__main__":
    bg_folder = r"E:\Dataset\All Video BG\Watermarked\Vertical" # Vertical
    fg_folder = r"E:\Dataset\All Audio Quotes\Video Clips (Full Letters)\Vertical\2s Added" # Vertical
    title_folder = r"E:\Dataset\All 200+ Stoic Quotes\Video Clips\Vertical\2s Added" # Vertical
    author_folder = r"E:\Dataset\All 200+ Stoic Quotes\Video Clips\Vertical\2s Added" # Vertical
    stoistica_folder = r"E:\Dataset\All STOISTICA Audio\Video Clips\Vertical\2s Added" # Vertical
    ...
    # Example usage
    resolution = "Vertical"  # Options: "Vertical", "Horizontal", "Square"
...
altbg_category = "Rock"  # Replace with "Exercise", "Hiking", "Ocean", "God", "Monk", "Rock","TimeLapse"
create_final_video_sequence(bg_folder, fg_folder, title_folder, author_folder, stoistica_folder, output_folder, resolution, duration, iterations, quality, bg_music, bg_music_volume, final_audio_volume, altbg_category)
os.remove("concat_list.txt")