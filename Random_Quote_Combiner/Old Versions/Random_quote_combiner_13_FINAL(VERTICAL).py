...
def ensure_unique_bg_videos(duration_folder, used_videos, required_duration):
    ...
    while total_bg_duration < required_duration and max_attempts > 0:
        try:
...
def merge_videos(bg_paths, fg_path, output_video_path, zoom=1.0, x_offset=0, y_offset=0, quality="Default"):
    ...
    try:
        ...
        # Downsample foreground audio if higher than 44.1kHz
        downsampled_fg_path = fg_path  # Default to the original path if no downsampling needed
        command = [
            "ffmpeg", "-i", fg_path, "-hide_banner", "-loglevel", "error",
            "-select_streams", "a", "-show_entries", "stream=sample_rate", "-of", "csv=p=0"
        ]
        sample_rate_output = subprocess.run(command, capture_output=True, text=True).stdout.strip()

        if sample_rate_output and int(sample_rate_output) > 44100:
            downsampled_fg_path = "downsampled_fg.mp4"
            command = [
                "ffmpeg", "-i", fg_path, "-ar", "44100", "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-y", downsampled_fg_path
            ]
            subprocess.run(command, check=True)
        ...
        command = [
            "ffmpeg", "-hwaccel", "cuda", "-i", bg_concat_path, "-i", downsampled_fg_path,
            "-filter_complex",
    ...
    if downsampled_fg_path != fg_path:
        os.remove(downsampled_fg_path)
...
def create_final_video_sequence(bg_folder, fg_folder, title_folder, author_folder, stoistica_folder, output_folder, duration, iterations, quality, bg_music, bg_music_volume, final_audio_volume, altbg_category=None):
    ...
    for i in range(iterations):
        try:
            ...
            # Rename to final output name with timestamp
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            final_name = os.path.join(output_folder, f"final_video_{timestamp}_{i}.mp4")
...
if __name__ == "__main__":
    ...
    title_folder = r"E:\Dataset\All Authors - 44.1khz\Test Vertical (44.1khz)"
    author_folder = r"E:\Dataset\All Authors - 44.1khz\Test Vertical (44.1khz)"
    ...
    iterations = 5  # Number of iterations to generate
    ...
    altbg_category = "Time"  # Replace with "Exercise", "Hiking", "Ocean", "God", "Monk", "Rock","Time" or None
