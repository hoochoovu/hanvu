...
def check_gpu_status():
    """Monitor GPU usage and log any issues."""
    try:
        gpu_status = subprocess.run(["nvidia-smi"], capture_output=True, text=True)
        logging.info(f"GPU Status: {gpu_status.stdout}")
...
def select_random_video_without_reuse(folder, used_videos):
    ...
    if not videos:
        logging.warning(f"No more unused videos in {folder}. Resetting selection.")
...
def merge_videos(bg_paths, fg_path, output_video_path, zoom=1.0, x_offset=0, y_offset=0, quality="Default", gpu_index=0):
    ...
    try:
        ...
        bitrate = QUALITY_SETTINGS.get(quality, "2000k")
        ...
        command = [
            "ffmpeg", f"-hwaccel", "cuda", f"-gpu", str(gpu_index), "-i", bg_concat_path, "-i", fg_path,
            "-filter_complex",
            ...
            "-map", "[video]", "-map", "1:a?",
            "-c:v", "av1_nvenc", "-preset", "slow", "-b:v", bitrate, "-c:a", "aac", "-b:a", "192k",
            "-progress", "pipe:1",
            "-y", output_video_path
        ]
        ...
        total_duration = sum(get_video_duration(bg) for bg in bg_paths)
        pbar = tqdm(total=int(total_duration), unit='sec')

        for line in process.stdout:
            if "out_time_ms" in line:
                time_ms = int(line.split("=")[1])
                pbar.update(int(time_ms / 1000000) - pbar.n)
...
def merge_audio_to_video(video_path, bg_music, bg_music_volume=0, final_audio_volume=0, used_audios=set()):
    ...
    command = [
        ...
        "-map", "0:v", "-map", "[out]", "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-y", output_video_path
    ]
...
def create_final_video_sequence(bg_folder, fg_folder, title_folder, author_folder, stoistica_folder, book_folder, output_folder, duration, iterations, quality, bg_music, bg_music_volume, final_audio_volume, altbg_category=None):
    ...
    for i in tqdm(range(iterations), desc="Creating videos"):
        try:
            ...
            command = [
                ...
                "-c:v", "av1_nvenc", "-preset", "slow", "-y", final_video_path
            ]
...
if __name__ == "__main__":
    ...
    duration = 40271  # Options: duration in seconds (e.g., 59, 119, 599, 30 * 60, 60 * 60, 40271)
    iterations = 1  # Number of iterations to generate
    quality = "High"  # Options: "Default", "High", "Higher", "Intense", "Extreme"
