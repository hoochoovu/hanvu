...
def insert_video_metadata(video_path, title, artist, genre, copyright, description, gpu_id=0):
    """Inserts metadata into the video file using ffmpeg."""
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)  # Set GPU ID
...
def trim_video_to_duration(input_video, output_video, duration, gpu_id=0):
...
def merge_videos(bg_paths, fg_path, output_video_path, gpu_id=0, zoom=1.0, x_offset=0, y_offset=0, quality="Default"):
...
def merge_audio_to_video(video_path, bg_music, gpu_id=0, bg_music_volume=0, final_audio_volume=0, used_audios=set()):
...
def create_final_video_sequence(bg_folder, fg_folder, title_folder, author_folder, stoistica_folder, book_folder, output_folder, duration, iterations, quality, bg_music, bg_music_volume, final_audio_volume, altbg1_folder, altbg2_folder):
    ...
    for i in range(iterations):
        try:
            ...
            bg_videos = []
            total_bg_duration = 0
            use_altbg1 = True  # Flag to alternate between altbg1 and altbg2

            # First, use bg_folder for the initial background video
            ...
            # Then alternate between altbg1_folder and altbg2_folder for subsequent background videos
            while total_bg_duration < duration:
                if use_altbg1:
                    selected_video = select_random_video_without_reuse(altbg1_folder, used_videos)
                else:
                    selected_video = select_random_video_without_reuse(altbg2_folder, used_videos)
                
                bg_videos.append(selected_video)
                total_bg_duration += get_video_duration(selected_video)
                use_altbg1 = not use_altbg1  # Toggle between altbg1 and altbg2
...
if __name__ == "__main__":
    ...
    duration = 59  # Options: duration in seconds (e.g., 59, 119, 599, 30 * 60, 60 * 60, 23795 ,28799, 32948, 40270)
    ...
    altbg2_folder = r"E:\Dataset\All Video BG\Watermarked\Vertical\Smoke"
    ...
    create_final_video_sequence(
        ...
        book_folder, output_folder, duration, iterations, quality,
        bg_music, bg_music_volume, final_audio_volume,
        altbg1_folder, altbg2_folder
    )