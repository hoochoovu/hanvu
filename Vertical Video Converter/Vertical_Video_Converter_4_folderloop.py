    ...
    if os.path.isdir(input_subfolder):
        ...
        for filename in os.listdir(input_subfolder):
            if filename.endswith('.mp4'):
                ...
                output_path = os.path.join(output_subfolder, os.path.splitext(filename)[0] + '_scaled_AV1.mp4')
                ...
                scaled_width = int(1080 * scale_x)
                scaled_height = int(1920 * scale_y)

                # Ensure cropped dimensions are within the scaled dimensions
                crop_width = min(scaled_width, 1080)  # Correct order: width, then height
                crop_height = min(scaled_height, 1920) # Correct order: width, then height
