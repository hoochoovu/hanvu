...
def merge_videos(bg_concat_path, fg_path, output_video_path, crop_top=0, crop_bottom=0,
                 transparency_color='none', alpha=1.0, quality="Default", position='center'):
    ...
    try:
        ...
        if position == 'center':
            ...
            # Set custom x_offset and y_offset if needed
        ...
        transparency_filter = ''
        if transparency_color.lower() == 'none':
            transparency_filter = ''
        else:
            # Check if transparency_color is a valid hex code
            if re.match(r'^[0-9a-fA-F]{6}$', transparency_color):
                # It's a hex code
                color_value = f'0x{transparency_color}'
            else:
                # Assume it's a color name
                color_value = transparency_color.lower()
            transparency_filter = f'colorkey={color_value}:0.1:0.1,'
...
def select_random_audio(folder, used_audios):
    """Selects a random audio file from a folder, including subfolders, ensuring no repeats until all have been used."""
    audios = [os.path.join(root, f)
              for root, _, files in os.walk(folder)
              for f in files
              if f.lower().endswith(('.mp3', '.wav'))]
    unused_audios = [a for a in audios if a not in used_audios]
    if not unused_audios:
        logging.info(f"All audio files have been used. Resetting used_audios.")
        used_audios.clear()
        unused_audios = audios.copy()
    selected_audio = random.choice(unused_audios)
    used_audios.add(selected_audio)
    return selected_audio

def merge_audio_to_video(video_path, bg_music=None, bg_music_volume=0, final_audio_volume=0, used_audios=None):
    ...
    if bg_music is None:
        ...
        if os.path.isdir(bg_music):
            ...
            # Ensure that single bg_music files also respect the used_audios logic
            if used_audios is not None:
                if bg_music_path in used_audios:
                    logging.info(f"All background music files have been used. Resetting used_audios.")
                    used_audios.clear()
                used_audios.add(bg_music_path)
...
def create_final_video_sequence(fg_folder, altbg_folders, output_folder, iterations, quality, bg_music=None, bg_music_volume=0, final_audio_volume=0, crop_top=0, crop_bottom=0, transparency_color='none', alpha=1.0, position='center'):
    ...
    used_fg_videos = set()
    used_bg_videos = set()
    used_audios = set()  # Initialize used_audios here
    ...
    for i in range(iterations):
        try:
            ...
            final_name = os.path.join(
                output_folder,
                f"Stoic Radio_Podcast_Video_{fg_name_part}_{timestamp}_{i+1}.mp4"
            )
...
if __name__ == "__main__": 
    ...
    # Input folders
    fg_folder = r"E:\Dataset\All Notebook Audio\Podcast Clips Blanks\Vertical"
    ...
    altbg5_folder = r"E:\Dataset\All Video BG\Watermarked\Vertical\Smoke\All Meditation"
    altbg6_folder = "E:\Dataset\All Video BG\Watermarked\Vertical\All Artwork"
    # Add more altbg folders as needed

    altbg_folders = [altbg1_folder, altbg2_folder, altbg3_folder, altbg4_folder, altbg5_folder]

    output_folder = "podcast_maker_output"

    iterations = 3 # Number of videos to create
    quality = "Extreme"  # Options: "Default", "High", "Higher", "Intense", "Extreme"

    bg_music = r"E:\Dataset\ALL BG Music\58 mins Sub-Conscious Sleep Music 58 mins"  # Set to path of bg music or None to not use bg music
    bg_music_volume = 3  # Volume adjustment in decibels, negative values reduce volume
    ...
    transparency_color = '000000'  # Hex code for the desired transparency color 000000 black and ffffff white
